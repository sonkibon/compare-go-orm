...
type DepartmentQuery struct {
	...
	withDeptManager *DeptManagerQuery
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withDeptManager: dq.withDeptManager.Clone(),
...
// WithDeptManager tells the query-builder to eager-load the nodes that are connected to
// the "dept_manager" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DepartmentQuery) WithDeptManager(opts ...func(*DeptManagerQuery)) *DepartmentQuery {
	query := (&DeptManagerClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withDeptManager = query
	return dq
}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withDeptManager; query != nil {
		if err := dq.loadDeptManager(ctx, query, nodes,
			func(n *Department) { n.Edges.DeptManager = []*DeptManager{} },
			func(n *Department, e *DeptManager) { n.Edges.DeptManager = append(n.Edges.DeptManager, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadDeptManager(ctx context.Context, query *DeptManagerQuery, nodes []*Department, init func(*Department), assign func(*Department, *DeptManager)) error {
	...
	query.Where(predicate.DeptManager(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.DeptManagerColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.department_dept_manager
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_dept_manager" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_dept_manager" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
