...
func (eq *EmployeeQuery) Clone() *EmployeeQuery {
	...
	return &EmployeeQuery{
		...
		withDeptManager: eq.withDeptManager.Clone(),
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Employee, error) {
	...
	if query := eq.withDeptManager; query != nil {
		if err := eq.loadDeptManager(ctx, query, nodes,
			func(n *Employee) { n.Edges.DeptManager = []*DeptManager{} },
			func(n *Employee, e *DeptManager) { n.Edges.DeptManager = append(n.Edges.DeptManager, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EmployeeQuery) loadDeptManager(ctx context.Context, query *DeptManagerQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *DeptManager)) error {
	...
	query.Where(predicate.DeptManager(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.DeptManagerColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.employee_dept_manager
		if fk == nil {
			return fmt.Errorf(`foreign-key "employee_dept_manager" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "employee_dept_manager" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
