...
func (dmq *DeptManagerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*DeptManager, error) {
	...
	if dmq.withEmployee != nil || dmq.withDepartment != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, deptmanager.ForeignKeys...)
	}
...
func (dmq *DeptManagerQuery) loadEmployee(ctx context.Context, query *EmployeeQuery, nodes []*DeptManager, init func(*DeptManager), assign func(*DeptManager, *Employee)) error {
	...
	nodeids := make(map[int][]*DeptManager)
	for i := range nodes {
		if nodes[i].employee_dept_manager == nil {
			continue
		}
		fk := *nodes[i].employee_dept_manager
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "employee_dept_manager" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dmq *DeptManagerQuery) loadDepartment(ctx context.Context, query *DepartmentQuery, nodes []*DeptManager, init func(*DeptManager), assign func(*DeptManager, *Department)) error {
	...
	nodeids := make(map[int][]*DeptManager)
	for i := range nodes {
		if nodes[i].department_dept_manager == nil {
			continue
		}
		fk := *nodes[i].department_dept_manager
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "department_dept_manager" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dmq *DeptManagerQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(deptmanager.Table, deptmanager.Columns, sqlgraph.NewFieldSpec(deptmanager.FieldID, field.TypeInt))
	_spec.From = dmq.sql
